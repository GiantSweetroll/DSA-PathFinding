...
public:
    ...
    void generate() {
       ...
       // Clears the main seating layout
        ...
        int repetition = clamp(customPassengerList.size(), 0, (length * width));
        // Adds all the custom passengers to the BST
        for (int i = 0; i < repetition; i++) {
            sortedPassengers.insert(customPassengerList.get(i));
        }

        repetition = clamp(passengerList.size(), 0, (length * width) - sortedPassengers.getSize());
        ...
        repetition = clamp(emptySeats, 0, numeric_limits<int>().max());
        for (int i = 0; i < repetition; i++) {
